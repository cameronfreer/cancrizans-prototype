#!/usr/bin/env python3
"""
Auto-generate CLI reference documentation with examples
"""
...
def generate_cli_reference():
    """Generate CLI reference with examples."""

    content = f"""# CLI Reference
...
def main():
    ...
    cli_docs = generate_cli_reference()

    # Write to file
    output_path = Path(__file__).parent.parent / 'docs' / 'CLI_REFERENCE.md'
    ...
    print(f"  Size: {len(cli_docs)} characters")
    print(f"  Lines: {len(cli_docs.splitlines())}")
